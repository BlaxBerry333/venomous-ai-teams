...
- **跨任务 `/compact` 建议** — 场景结束后若下一任务与当前无关联（换技术领域/换目录/换项目），角色会主动建议 `/compact`。Claude Code 会在 compact 后自动重新附加 `/编程专家` 定义（< 5000 tokens，完整保留），无需重新调用命令。场景四审查报告仅在对话中，若需保留，compact 前先让角色写入文件
