...
- 改完 `.claude/{agents,commands,hooks}/`、`.claude/settings.json`，或 `teams/*/.claude/{agents,commands,hooks,settings.json}`，或 `setup.sh` 后，宣告「完成」前必须 spawn `开发审查员` sub-agent，至少 2 个独立实例都零发现才算真完成（可并行）；其他改动跳过
- spawn 开发审查员时按改动量给核查点（小改 2-3 项、大改 5-6 项）+ 一句「按你的边界跑」；硬约束在开发审查员 .md 自身（≤8 工具调用 / 2 bug 即停）；禁用「挑剔到底 / 穷举攻击面 / 自己造场景」等放大搜索半径的措辞
- **开发审查员发现 ≠ 真 bug**：每条发现先过「不改会怎样 / 影响什么」一道复核——是真坏行为 / 是注释债 / 是误判机制？误判和无害债不修，避免越改越乱
- 不准凭印象说"应该没事"
...
「不改会怎样」一句话答真坏行为 / 注释债 / 无影响（无影响时该改动应被回滚）。影响方向：✅ 正向 / ⚠️ 中性 / ❌ 负向。**只列实际改了的项；驳回 / 误判 / 决定不改的不进表**——表是改动汇报不是决策日志。
