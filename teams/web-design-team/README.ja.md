...
- **Playwright**（**強く推奨**、技術的には任意）：参考分解時に初期画面 HTML + スクロールフレームを取得。無ければ WebFetch にフォールバックして静的 HTML のみとなり、**スクロール / hover / mousemove 系動効は推測頼り**——分解者の価値の大半が損なわれる。対象プロジェクト内で一度だけ：`npm i -D playwright && npx playwright install chromium`（分解スクリプトはプロジェクト内 node_modules を解決するため、グローバルインストールは不可）
